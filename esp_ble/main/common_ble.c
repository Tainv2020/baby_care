...
#define BATTERY_READ_CHAR_UUID    0x2A19
...
ESP_EVENT_DEFINE_BASE(EVENT_BLE);
...
// uint8_t esp_bd_addr[GATTS_SUPPORT][ESP_BD_ADDR_LEN] =
// {
//     {0xEF,0x6A,0xF2,0x03,0xAD,0x7B},
//     {0xD6,0xFE,0xFA,0xF6,0x68,0x26}
// };

static esp_bd_addr_t device_to_connect = {0};
...
static esp_bt_uuid_t battery_filter_service_uuid = {
    .len = ESP_UUID_LEN_16,
    .uuid = {.uuid16 = BATTERY_SERVICE_UUID,},
};

static esp_bt_uuid_t battery_filter_char_uuid = {
    .len = ESP_UUID_LEN_16,
    .uuid = {.uuid16 = BATTERY_READ_CHAR_UUID,},
};
...
{
    return (uint32_t) (p_data[2]*256+p_data[1]);
}
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_CONNECT_EVT:{
        esp_event_post(EVENT_BLE, EVENT_BLE_CONNECTED, NULL, 0, portMAX_DELAY);
    ...
    case ESP_GATTC_DIS_SRVC_CMPL_EVT:
        ...
        esp_ble_gattc_search_service(gattc_if, param->cfg_mtu.conn_id, &battery_filter_service_uuid);
        break;
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
        ...
        if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16 && p_data->search_res.srvc_id.uuid.uuid.uuid16 == BATTERY_SERVICE_UUID) {
            ESP_LOGI(GATTC_TAG, "service found");
            get_server = true;
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT:
        ...
        if (get_server){
            ...
            if (count1 > 0){
                char_elem_result = (esp_gattc_char_elem_t *)malloc(sizeof(esp_gattc_char_elem_t) * count1);
                if (!char_elem_result){
                    ESP_LOGE(GATTC_TAG, "gattc no mem");
                }else{
                    status = esp_ble_gattc_get_char_by_uuid( gattc_if,
    ...
    case ESP_GATTC_DISCONNECT_EVT:
        esp_event_post(EVENT_BLE, EVENT_BLE_DISCONNECTED, NULL, 0, portMAX_DELAY);
        connect = false;
        get_server = false;
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT: {
        ESP_LOGI(GATTC_TAG, "set param scan success");
        break;
    }
    case ESP_GAP_BLE_SCAN_START_COMPLETE_EVT:
        esp_event_post(EVENT_BLE, EVENT_BLE_START_SCAN, NULL, 0, portMAX_DELAY);
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        esp_event_post(EVENT_BLE, EVENT_BLE_SCAN_DONE, NULL, 0, portMAX_DELAY);
    ...
    case ESP_GAP_BLE_SCAN_STOP_COMPLETE_EVT:
        esp_event_post(EVENT_BLE, EVENT_BLE_STOP_SCAN, NULL, 0, portMAX_DELAY);
...
{
    if(device_to_connect[0] != 0x00){
        ESP_LOGW(GATTC_TAG,"Try connect to %02x %02x %02x %02x %02x %02x..", device_to_connect[0],device_to_connect[1],device_to_connect[2]   \
                                                                     , device_to_connect[3],device_to_connect[4], device_to_connect[5]);
        esp_ble_gap_start_scanning(duration);
        timeout_for_read_data_start();
    }
    else
        ESP_LOGE(GATTC_TAG, "NOT SET DEVICE TO CONNECT");
}
...
{
    memcpy(device_to_connect, ble_addr, 6);
}
...
bool app_ble_add_device_to_table(int ble_index, uint8_t * ble_addr)
{
    ...
    for(int i=0;i<GATTS_SUPPORT;i++)
    {
        if(memcmp(ble_device_table[i].ble_addr, ble_addr,6) == 0){
            is_exist = true;
            break;
        }
    }

    if((!is_exist) && (!ble_device_table[ble_index].slot_is_used)){
        memcpy(ble_device_table[ble_index].ble_addr, ble_addr, 6);
        ble_device_table[ble_index].slot_is_used = true;
    }
